"""Membership model — links user to group with status and admin flag."""
import uuid
...
class Membership(Base):
    ...
    status = Column(Enum('pending', 'approved', 'rejected', name='membership_status_enum'),
    ...
    # Relationships
    user = relationship('User', foreign_keys=[user_guid], back_populates='memberships')
    group = relationship('Group')
    decided_by = relationship('User', foreign_keys=[decided_by_guid])

    def __repr__(self):
        return f'<Membership user={self.user_guid} group={self.group_guid} status={self.status}>'
