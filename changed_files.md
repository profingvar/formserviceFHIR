...
| Date       | File                          | Action  |
|------------|-------------------------------|---------|
| 2026-04-16 | sso.pdhc/app/src/app.py       | Ticket #74 (#58 followup): re-applied HTTPException pass-through in `@app.errorhandler(Exception)` that had been lost when local was synced server→local during #70. Added `from werkzeug.exceptions import HTTPException` import + `if isinstance(e, HTTPException): return e` before generic 500 branch. Deployed via scp + SIGHUP on master pid 1142 (no restart). Verified: `curl https://sso.pdhc.se/no-such-path` was 500, now 404; sso /api/health still 200; contract/plan/dashboard/rosetta SSO-dependent /health still 200. |
| 2026-04-16 | sso.pdhc/app/oath_overview.csv | Ticket #73: fixed `cgm.pdhc.pdhc.se` → `cgm.pdhc.se` (duplicate subdomain typo) + added 4 missing rows (cdr, dashboard, rosetta, forms) so the services-overview table covers every active pdhc service. Health URLs pulled from CLAUDE.md §3 (cdr/dashboard/rosetta use /healthz, forms uses /api/health). Deployed via scp; local and server sha match 6593e531. Server backup `/tmp/oath_overview.server.bak.20260416T201631Z.csv`. |
...
| 2026-04-16 | app/src/templates/su_admin.html | Ticket #62: removed phase-name datalist suggestions from the "Create Group" category field — the four options (`planning`, `request`, `provider`, `analysis`) kept priming SU admins to categorise groups with phase names and perpetuated the mental coupling that #57 and #60 broke at the data layer. Placeholder replaced with neutral organisational examples (`oncology`, `regional`, `research`). Help text below the form explicitly warns against using phase names as categories. |
| 2026-04-16 | app/src/templates/suggest_group.html | Ticket #62: same datalist removal on the public "Suggest Group" form; placeholder now shows neutral organisational examples. |
- 2026-04-16: app/src/app.py — ticket #70 adds CORS on /api/health (Access-Control-Allow-Origin https://www.pdhc.se + Methods GET + Vary: Origin + Cache-Control: no-store). Restarted via SIGHUP on master pid (no service interruption). Note: local had a ticket-#58 undeployed fix (HTTPException handler pass-through) that is NOT in this deploy — synced server→local first. Operator should decide separately whether to deploy the HTTPException fix.

| 2026-04-29 | app/src/models/external_partner.py | NEW — `ExternalPartner` + `ExternalPartnerAudit` ORM models per `plans/external_partners_plan.md` §3. Two tables added to Base.metadata: `external_partner` (current state, with auth/credential hashes, allowed_scopes/services, status enum, lifecycle timestamps, FK to users.guid for created_by) and `external_partner_audit` (append-only, every state change with before/after JSON and actor user). `public_dict()` / `admin_dict()` projections enforce the leak boundary (no hashes, no contact info on public). |
| 2026-04-29 | app/src/models/__init__.py | Re-exported `ExternalPartner` + `ExternalPartnerAudit` so `init_db.create_all()` picks them up. |
| 2026-04-29 | app/src/routes/partners.py | NEW — three route groups on a single blueprint: public (`GET /api/public/partner/<guid>`), internal service-to-service (`POST /api/internal/partner/<guid>/validate`), and SU admin CRUD (`/api/admin/partners[/<guid>][/rotate|/suspend|/reactivate|/revoke|/audit]` + `_meta/catalogue`). Closed-set scope catalogue (`SCOPE_CATALOGUE`) and known-services list (`KNOWN_SERVICES`) gate what the SU can register. Credentials minted server-side via `secrets.token_urlsafe(32)` and hashed via the same `hash_password` used for users; cleartext returned exactly once in the response body of POST /partners and POST /partners/<guid>/rotate. Plan §4.1, §5, §6.2. |
| 2026-04-29 | app/src/app.py | Registered `partners_bp` blueprint and added it to the CSRF-exempt list (it uses Bearer tokens, not cookies). |
| 2026-04-29 | app/src/templates/su_admin.html | Replaced the broken Service Key Management block (the one in the screenshot perpetually showing "No keyed services configured. Set KEYAUTH_SERVICE_*_URL and KEYAUTH_SERVICE_*_KEY in .env.") with a working External Partners panel: register form (display_name / country / org_number / contact / auth_kind / expires / description / allowed services + scopes from the catalogue), table of existing partners with status badges + per-row Rotate / Suspend / Reactivate / Revoke buttons, secret-shown-once banner on register/rotate. Plan §5.2. The legacy KEYAUTH_SERVICE_* env-pair scheme + the `KEYED_SERVICES` config block in `src/config.py` are left in place for now; deferred cleanup tracked as plan §7 step 6 (operator confirms no env vars in prod first). |
| 2026-04-29 | app/tests/test_partners.py | NEW — 22 tests covering create→returns secret once + persists hash only, public lookup leaks nothing sensitive, internal validate accepts good / rejects bad / rejects suspended-revoked-expired / requires service key, edit allows mutable / blocks immutable, rotate replaces credential and invalidates old, suspend→reactivate cycle, revoke is irreversible, audit log captures every state change with actor, scope/service catalogues round-trip. All 22 pass; full suite 253/253. |
| 2026-04-29 | app/tests/test_models.py | EXPECTED_TABLES extended with `external_partner` and `external_partner_audit` so `test_table_count` stays accurate. |
